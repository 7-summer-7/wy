...
def main(force: bool = False):
    ...
    # split (this is done on the original raw data, without any imputation or scaling)
    ...
    # Save raw data (no imputation, no scaling)
    pd.DataFrame(X_train_raw, columns=X_h1.columns).to_csv(os.path.join(DAT_DIR, "X_train_raw.csv"), index=False)
    pd.DataFrame(X_test_raw, columns=X_h1.columns).to_csv(os.path.join(DAT_DIR, "X_test_raw.csv"), index=False)
    pd.DataFrame(X_ext_raw, columns=X_h1.columns).to_csv(os.path.join(DAT_DIR, "X_ext_raw.csv"), index=False)
    ...
    # 5) final training matrices (standardized + imputed numeric for CONTINUOUS_COLS) + BINARY_COLS + OHE
    ...
    # Save inverse-transformed data (original scale)
